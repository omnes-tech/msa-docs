---
title: Verificando Status da Carteira
description: Verifique a existência da carteira, status de implantação e recupere informações da carteira usando a API MSA.
---

# Verificando Status da Carteira

A API MSA fornece endpoints para verificar a existência da carteira, confirmar o status de implantação e recuperar informações da carteira. Isso é essencial para gerenciar o ciclo de vida da carteira e garantir que as carteiras sejam implantadas corretamente.

## Visão Geral

Você pode verificar o status da carteira por:
- **Salt**: Verificar se uma carteira existe para um determinado salt
- **Endereço**: Verificar se um endereço é uma carteira MSA válida
- **Verificações em Lote**: Verificar múltiplas carteiras de uma vez

## Verificação Básica de Carteira
...
- **[Criando Carteiras](/docs/wallet-management/creating-wallets)** - Implante novas carteiras
- **[Prevendo Endereços](/docs/wallet-management/predicting-addresses)** - Calcule endereços de carteira
- **[Tipos de Custody](/docs/wallet-management/custody-types)** - Entenda configurações de carteira
